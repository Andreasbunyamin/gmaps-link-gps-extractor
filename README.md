# Google Maps Link â†’ GPS Coordinates
...
| PLACE NAME | LINK GMAPS | â†’ | Latitude | Longitude | Source |
|---|---|---|---|---|---|
...
|---|---|---|
| `!3d<lat>!4d<lng>` | the **map pin** â€” the place itself | exact |
| `@<lat>,<lng>,17z` | the **camera** â€” where the viewport is centred | a few metres off |
...
| â€” | *(blank)* | nothing usable â€” blank cell, junk text, dead link | â€” |
...
| `--delay` | `0` | Seconds between rows â€” raise it for large batches |
| `--limit` | none | Process only the first N rows |
| `--csv` | off | Also write a `.csv` beside the `.xlsx` |
| `-q, --quiet` | off | Suppress per-row output |

Every default can also be set through `.env` â€” see `.env.example`.
</details>
...
```
â”œâ”€â”€ src/gmaps_gps/
â”‚   â”œâ”€â”€ url_parser.py      # pure extraction logic â€” no browser, no network
â”‚   â”œâ”€â”€ scraper.py         # Selenium session, redirect + consent handling
â”‚   â”œâ”€â”€ pipeline.py        # batch processing over a DataFrame
â”‚   â”œâ”€â”€ exporter.py        # formatted Excel output
â”‚   â””â”€â”€ cli.py             # command-line interface
â”œâ”€â”€ notebooks/             # step-by-step walkthrough (Colab-ready)
â”œâ”€â”€ data/                  # synthetic sample dataset
â”œâ”€â”€ tests/                 # 43 tests, browser-free
â”œâ”€â”€ scripts/               # regenerate the sample data and the docs diagram
...
project is unit-testable without launching Chrome â€” and the whole test suite runs
...
`data/sample_tourist_places.xlsx` holds 36 public Indonesian tourist destinations â€”
Borobudur, Tanah Lot, Danau Toba, Raja Ampat and so on â€” generated by
...
rendered table â€” every row, every link, every coordinate â€” inside the `.ipynb` file,
...
- **Google Maps is not a public API.** URL formats change without notice. The
  ...
  extraction â€” the `Source` column is how you would notice.
...
Python Â· pandas Â· Selenium Â· XlsxWriter Â· pytest
